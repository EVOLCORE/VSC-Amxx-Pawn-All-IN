...
function registerAmxxBuilderIntegration(context) {
    ...
    function runBuildTerminal(folder, manifestPath) {
        ...
        const terminal = createTerminal(workspaceRoot);
    ...
    function runInit(folder = getActiveWorkspaceFolder()) {
        if (!folder?.uri?.fsPath) {
            const { showTimedErrorMessage } = getNotificationHelpers();
            showTimedErrorMessage(vscode, t('amxxBuilder.noWorkspace'));
            return;
        }
        const terminal = createTerminal(folder.uri.fsPath);
        terminal.show(true);
        terminal.sendText(`${AMXX_BUILDER_EXECUTABLE} init`);
    }
